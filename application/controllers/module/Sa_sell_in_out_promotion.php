...
class Sa_sell_in_out_promotion extends CI_Controller {
	...
	public function load_calculate($sheet, $emp_number){
	...
	public function load_sell_out($emp_number) {

		$filter_select_promo = ['start_date', 'end_date', 'customer_code', 'promotion_no', 'model', 'price_promotion', 'gift'];
		$data_calculate = $this->gen_m->filter_select('sa_calculate_promotion', false, $filter_select_promo, ['pr' => $emp_number]);
		...
		foreach ($data_sell_out_all as $item) {
			$organized_data[$item->model_suffix_code][$item->customer_model][$item->txn_date][] = $item;
		}
		...
		foreach ($all_models as $model) {
			...
			$culculate_price_promotions = $this->gen_m->filter_select('sa_calculate_promotion', false, ['price_promotion', 'model', 'start_date', 'end_date'], ['model' => $model, 'pr' => $emp_number]);
				...
				if (!empty($culculate_price_promotions)) {
						...
						if (!in_array($promotion, $promotions_by_type['basica'])) {
							...
							if ($promotion->start_date == $mes_inicio_promo && $promotion->end_date == $mes_fin_promo) {
								...
								if (!$is_additional && count($basic_ranges) == 2) {
									...
									//---------------------------------
									$start_in_basic1_rev = strtotime($promotion->start_date) >= strtotime($basic_ranges[1]['start']) && strtotime($promotion->end_date) > strtotime($basic_ranges[1]['end']);
									$end_in_basic1_rev = strtotime($promotion->end_date) >= strtotime($basic_ranges[0]['start']) && strtotime($promotion->end_date) <= strtotime($basic_ranges[0]['end']);
									
									$start_in_basic2_rev = strtotime($promotion->start_date) >= strtotime($basic_ranges[1]['start']) && strtotime($promotion->end_date) <= strtotime($basic_ranges[1]['end']);
									$end_in_basic2_rev = strtotime($promotion->end_date) >= strtotime($basic_ranges[1]['start']) && strtotime($promotion->end_date) <= strtotime($basic_ranges[1]['end']);
									
									if ((($start_in_basic1 && $end_in_basic2) || ($start_in_basic2 && $end_in_basic1)) || ($start_in_basic1_rev && $end_in_basic1_rev)) {
										$is_mixta = true;
									}
								}
							}
					...
					$culculate_price_promotions_bundles = $this->gen_m->filter_select('sa_calculate_promotion', false, ['price_promotion', 'model', 'start_date', 'end_date'], ['gift' => $model, 'pr' => $emp_number]);
			...
			if (isset($organized_data[$model])) {
				foreach ($organized_data[$model] as $customer_model => $dates_data) {
					for ($current_date = $from_date; $current_date <= $to_date; $current_date = date('Y-m-d', strtotime($current_date . ' +1 day'))) {
						if (isset($dates_data[$current_date])) {
							$daily_stock_sum = 0;
							$first_item_base = null;
							$has_nonzero_sellout = false;
							$current_day_records = [];

							foreach ($dates_data[$current_date] as $item_out) {
								$base_data = [
								...
								if (!empty($promotions_by_type['basica'])) {
									...
									if ($is_orient === true){
										$relacion = isset($item_out->ticket) && isset($promotions_by_type['basica'][0]->price_promotion) && $item_out->ticket != 0 ? ($item_out->sellout_amt/$item_out->sellout_unit) <= 1.05*($promotions_by_type['basica'][0]->price_promotion) : 0; // Default value if ticket or price is missing
									...
									}elseif($item_out->customer === 'PE000952001B' && strpos($item_out->cust_store_name, 'B2B2C') !== false){
										$relacion = isset($item_out->ticket) && isset($promotions_by_type['basica'][0]->price_promotion) && $item_out->ticket != 0 ? ($item_out->sellout_amt/$item_out->sellout_unit) <= 1.05*($promotions_by_type['basica'][0]->price_promotion) : 0; // Default value if ticket or price is missing
										$target_flag_base = ($relacion === true) ? 'PROMO APPLIED' : 'PROMO NO APPLIED';
									}else{
										$relacion = isset($item_out->ticket) && isset($promotions_by_type['basica'][0]->price_promotion) && $item_out->ticket != 0 ? (int)$item_out->ticket <= 1.05*($promotions_by_type['basica'][0]->price_promotion) : 0; // Default value if ticket or price is missing
										$target_flag_base = ($relacion === true) ? 'PROMO APPLIED' : 'PROMO NO APPLIED';
									}
									
									if ($item_out->ticket == 0) $target_flag_base = 'PROMO APPLIED';
									
									if (($relacion >= 1) && (stripos($item_out->cust_store_name, 'WEB') !== false || stripos($item_out->cust_store_name, 'web') !== false || stripos($item_out->cust_store_name, 'b2b2c') !== false || stripos($item_out->cust_store_name, 'VIRTUAL') !== false)) {
								...
								foreach ($unique_promotions as $promotion) {
									if ($promo_index <= 10) {
										...
										if (isset($item_out->ticket) && isset($promotion->price_promotion) && $item_out->ticket != 0) {
											...
											if ($is_orient === true){
												$relacion = ($item_out->sellout_amt/$item_out->sellout_unit) <= 1.05*($promotion->price_promotion);
												$target_flags[$promo_index] = ($relacion === true) ? $target_flag_base : 'PROMO NO APPLIED';
											}else{							
												$relacion = (int)$item_out->ticket <= 1.05*($promotion->price_promotion);
												$target_flags[$promo_index] = ($relacion === true) ? $target_flag_base : 'PROMO NO APPLIED';
											}
					...
					for ($past_date = date('Y-m-d', strtotime($from_date . ' -1 day')); strtotime($past_date) >= strtotime($past_start_date); $past_date = date('Y-m-d', strtotime($past_date . ' -1 day'))) {
						if (isset($dates_data[$past_date])) {
							$daily_stock_sum_past = 0;
							$has_past_nonzero_sellout_on_date = false;
							//$current_past_day_items = $organized_data[$model][$past_date] ?? continue;
							...
							//echo '<pre>'; print_r($other_promotions);
							foreach ($other_promotions as $promotion) {
							...
							foreach ($dates_data[$past_date] as $item) {
					...
					if ($found_first_nonzero_stock_date !== null && $first_past_item_base !== null) {
		...
		usort($all_promotion_data_to_insert, function ($a, $b) {
			// 1. Comparar por model_code
			$model_code_cmp = strcmp($a['model_code'], $b['model_code']);
			if ($model_code_cmp !== 0) {
				return $model_code_cmp;
			}

			// 2. Si los modelos son iguales, comparar por customer_model
			$customer_model_cmp = strcmp($a['customer_model'], $b['customer_model']);
			if ($customer_model_cmp !== 0) {
				return $customer_model_cmp;
			}

			// 3. Si los modelos y customer_model son iguales, comparar por txn_date
			$txn_date_cmp = strtotime($a['txn_date']) - strtotime($b['txn_date']);
			if ($txn_date_cmp !== 0) {
				return $txn_date_cmp;
			}
			
			// 4. Si todo lo anterior es igual, priorizar el registro de STOCK
			$a_is_stock = isset($a['stock']) && ($a['stock'] !== 0 && $a['stock'] !== null);
			$b_is_stock = isset($b['stock']) && ($b['stock'] !== 0 && $b['stock'] !== null);
		...
		foreach ($all_promotion_data_to_insert as &$item) $item['pr'] = $emp_number;
		$batch_data = $all_promotion_data_to_insert;		
		$this->gen_m->insert_m("sa_sell_out_promotion", $batch_data);
	}
	
	public function load_sell_in($sheet, $emp_number){
		//excel file header validation
		$h = [
			trim($sheet->getCell('A1')->getValue()),
			trim($sheet->getCell('B1')->getValue()),
	...
	public function calcularStockAg($out_map, $model_calculate, $start_date_conv, $end_date_conv) {
		if (!empty($out_map[$model_calculate])) {
			...
			if ($indice_start_date !== -1) {			
				...
				// Buscar la primera fila con stock != 0
				...
				// Acumular stocks del mismo día y sellouts de días intermedios
				if ($indice_fecha_pasada !== -1) {
					// Acumular stock de la fila encontrada
					$stock_pasado = $fechas_modelo[$indice_fecha_pasada][2];

					// Acumular stocks de otras filas del mismo día
					for ($i = $indice_fecha_pasada - 1; $i >= 0; $i--) {
					...
					// Acumular sellouts de días intermedios (NO incluye la fecha del stock)
				...
				$fecha_pasada_comparacion = $fecha_pasada;
				...
				$fecha_futura_comparacion = null;

				// Bucle para encontrar la primera fila con stock != 0
				...
				// Si se encuentra una fecha con stock, acumula los valores
				if ($indice_fecha_futura !== -1) {
					// Primero, acumular el stock de la fila encontrada
					$stock_futuro = $fechas_modelo[$indice_fecha_futura][2];
					
					// Luego, revisar hacia adelante para acumular stocks de otras filas del mismo día
					for ($i = $indice_fecha_futura + 1; $i < count($fechas_modelo); $i++) {
						if ($fechas_modelo[$i][0] === $fecha_futura) {
							$stock_futuro += $fechas_modelo[$i][2]; // Acumula stocks
						} else {
							// Se detiene al llegar a un día diferente
							break;
						}
					}
					
					// Finalmente, acumular todos los sellouts desde la fecha de inicio hasta la fecha del stock
					// **incluyendo** los de la fecha del stock.
				...
				// La fórmula final suma los stocks acumulados con los sellouts acumulados.
				$stock_ag_futuro = ($fecha_futura !== null) ? $stock_futuro + $sellout_acumulado_futuro : null;
				$fecha_futura_comparacion = $fecha_futura;
	...
	public function generate_excel() {
		...
		$sell_in_data = $this->gen_m->filter_select('sa_sell_in_promotion', false, ['item_name', 'unit_selling_price', 'stock1', 'stock2', 'sales_order_qty', 'sales_yyyymmdd', 'invoice_no'], ['pr' => $emp_number]);
		$sell_map = [];
		...
		'target9_flag', 'promo9_price', 'target10_flag', 'promo10_price'], ['pr' => $emp_number]);
		$out_map = [];
		...
		$sell_out_all_data = $this->gen_m->filter('sa_sell_out_promotion', false, ['pr' => $emp_number], null, null, [['model_code', 'asc'], ['txn_date', 'asc']]);
		...
		$model_hiraoka = [];
		$count_basic = 0;
		$qty_model_tv = []; // list qty model tv if bundle exists
		$global_bundle = []; // bundles
		$model_tv_bundle = [];
		$dates_bundles = [];
                ...
                if ($sheet_name === 'CALCULATE') {
                    ...
                    foreach ($data as $index => $row) {
                        ...
                        if ($index > 0) { // Ignorar cabeceras
							...
							$new_sheet->setCellValue('M' . ($index + 1), $row[12]); // Llenar columna M
							$new_sheet->getStyle('M' . ($index + 1))
									  ->getNumberFormat()
							...
							//$previous_seq = [];
							//$previous_end_dates = [];
							if ($row[7] === 'PE000816001B') $is_hiraoka = true; // Hiraoka Cliente sheet
							$porcent = $row[12];
							$porcent_ns = str_replace("%", "", $porcent);
							$porcent_num = floatval($porcent_ns) / 100;
							$cost_prom = ((float)$row[11]/1.18)*(1-$porcent_num);
							...
							if (!isset($star_date_basic[$model_calculate])) $star_date_basic[$model_calculate] = 0;
							if (!isset($end_date_basic[$model_calculate])) $end_date_basic[$model_calculate] = 0;	
							if (!isset($start_date_pre_basic[$model_calculate])) $start_date_pre_basic[$model_calculate] = 0;
							if (!isset($end_date_pre_basic[$model_calculate])) $end_date_pre_basic[$model_calculate] = 0;
							/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////25.31
							...
							if(isset($star_date_basic[$model_calculate]) && isset($end_date_basic[$model_calculate]) && isset($end_date_pre_basic[$model_calculate]) && isset($start_date_pre_basic[$model_calculate]) && ($current_start_date >= $star_date_basic[$model_calculate] && $current_end_date <= $end_date_basic[$model_calculate]) || $current_start_date >= $start_date_pre_basic[$model_calculate] && $current_end_date <= $end_date_pre_basic[$model_calculate]){
								$flag_ocurrence = 1;
								...
								if ($flag_ocurrence == 1){
									...
									if ($current_start_date >= $star_date_basic[$model_calculate] && $current_end_date <= $end_date_basic[$model_calculate]){
										$ref_line = $seq_basic[$model_calculate];
									} elseif ($current_start_date >= $start_date_pre_basic[$model_calculate] && $current_end_date <= $end_date_pre_basic[$model_calculate]){
										$ref_line = $seq_pre_basic[$model_calculate];
									} 
									if (isset($date_additional[$model_calculate])){
										foreach ($date_additional[$model_calculate] as $item_add){
											if ($current_start_date >= $item_add[0] && $current_start_date <= $item_add[1]){
													if ($current_end_date <= $item_add[1]){
														$priority = "2-ADDITIONAL2";
														$cost_sellin = $item_add[3];
														$ref_line = $item_add[2]; //seq
													} elseif($current_end_date > $item_add[1]){
														$priority = "3-MIX";
														$ref_line = end($seq_basic_mix[$model_calculate]);
													}
											} else continue;
										}						
									}
									...
									//$start_date_add[$model_calculate][] = [$current_start_date, $row[0], $cost_prom]; // current_start_date, seq, cost_prom
									//$end_date_add[$model_calculate][] = [$current_end_date, $row[0], $cost_prom]; // current_end_date, seq, cost_prom
									$date_additional[$model_calculate][] = [$current_start_date, $current_end_date, $row[0], $cost_prom]; // current_end_date, seq, cost_prom
									$flag_ocurrence = 0;
								}
							...
							$current_start_date <= $start_date_pre_basic[$model_calculate] && $current_start_date <= $end_date_pre_basic[$model_calculate] && $current_end_date >= $start_date_pre_basic[$model_calculate])){
							...
							elseif($flag_ocurrence == 0 ){
								...
								if ($end_date_conv === $last_day_of_month) {
									...
									$start_date_pre_basic[$model_calculate] = $start_date_conv;
									$end_date_pre_basic[$model_calculate] = $end_date_conv;
									
									// seq - ref Line
									$seq_pre_basic[$model_calculate] = $row[0];
							...
							if($priority === 'BUNDLE'){
								$qty_bundle = $this->qty_calculate($out_map, $model_calculate, $start_date, $end_date, 1, $price_promotion);
								$new_sheet->setCellValue('AV' . ($index + 1), $qty_bundle); // Llenar columna AV
								//$qty = 0;
							...
							// Calculo NOTE
							if($qty == 0){
								if(!isset($out_map[$model_calculate])){
							...
							if ($stock_ag === null || $stock_ag === '') $stock_ag = 0;
							...
							if (!empty($model_bundle)){
								...
								$qty_model_tv[$model_bundle][$model_calculate] = $this->qty_calculate($out_map, $model_calculate, $start_date, $end_date, $flag_s_qty = 1, $price_promotion);
								$global_bundle[] = $model_bundle;
								$model_tv_bundle[$model_bundle][] = $model_calculate;
								$dates_bundles[$model_bundle] = ['start_date' => $start_date_conv, 'end_date' => $end_date_conv];
								//echo '<pre>'; print_r($global_bundle);
								//echo '<pre>'; print_r($qty_model_tv);
							}
						...
						if ($priority === '1-BASIC' || $priority === '3-MIX' || $priority === 'BUNDLE') {
							...
							if ($stock_column_index != -1 && isset($sell_in[$current_model])) {
								for ($index = $start_index; $index < count($sell_in[$current_model]); $index++) {
									...
									//log_message('info', "model: " . $current_model);
									//log_message('info', 'Remaning stock: ' . $remaining_stock);
		...
		if ($calculate_sheet) {
			...
			// calculate proporcion bundles unidades
			$total_model = [];
			$bundle_models_proportion = [];
			$global_bundle = array_values(array_unique($global_bundle));
			//$dates_bundles = array_unique($dates_bundles);
			$bundles_qty = [];
			for ($i = 0; $i <= count($global_bundle)-1; $i++){
				
				if (!isset($total_model[$global_bundle[$i]])){
					$total_model[$global_bundle[$i]] = 0;
				}
				foreach($qty_model_tv[$global_bundle[$i]] as $item){
					$total_model[$global_bundle[$i]] += $item;
				}
				
				$filter =[
						//'ticket' => 0,
						'pr' => $emp_number,
						'model_code' => $global_bundle[$i],
						'txn_date >=' => $dates_bundles[$global_bundle[$i]]['start_date'],
						'txn_date <=' => $dates_bundles[$global_bundle[$i]]['end_date']
					];
				$bundles_qty[$global_bundle[$i]] = $this->gen_m->filter_select('sa_sell_out_promotion', false, ['sellout_unit', 'target1_flag'], $filter);
				foreach ($bundles_qty[$global_bundle[$i]] as $item){
					if (!isset($uty_bundle[$global_bundle[$i]])) $uty_bundle[$global_bundle[$i]] = 0;
					if (isset($global_bundle[$i]) && $item->target1_flag !== 'PROMO NO APPLIED'){
						$uty_bundle[$global_bundle[$i]] += $item->sellout_unit;
					}
				}
				if(empty($bundles_qty[$global_bundle[$i]])) $uty_bundle[$global_bundle[$i]] = 0;
					
				for ($j = 0; $j < count($model_tv_bundle[$global_bundle[$i]]); $j++){	
					if (!isset($bundle_models_proportion[$global_bundle[$i]])) $bundle_models_proportion[$global_bundle[$i]] = [];
						if($total_model[$global_bundle[$i]] != 0){
							$bundle_models_proportion[$global_bundle[$i]][$model_tv_bundle[$global_bundle[$i]][$j]] =  $qty_model_tv[$global_bundle[$i]][$model_tv_bundle[$global_bundle[$i]][$j]]/$total_model[$global_bundle[$i]];
						} else $bundle_models_proportion[$global_bundle[$i]][$model_tv_bundle[$global_bundle[$i]][$j]] = 0;
					
					$bundles_unity[$global_bundle[$i]][$model_tv_bundle[$global_bundle[$i]][$j]] = $bundle_models_proportion[$global_bundle[$i]][$model_tv_bundle[$global_bundle[$i]][$j]] * $uty_bundle[$global_bundle[$i]];
				}
			}
			...
			foreach ($calculate_data as $row_index => $row) {
				...
				if(isset($cost_sellin_data[$model_bundle])){ // bundles unidades
					//echo '<pre>'; print_r($cost_sellin_data);
					...
					// Columna T
					$columnIndex = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(20 + 1);
					if ($bundles_unity[$model_bundle][$model_calculate] < 0) $bundles_unity[$model_bundle][$model_calculate] = 0;
					$calculate_sheet->setCellValue($columnIndex . ($row_index + 1), intval($bundles_unity[$model_bundle][$model_calculate]));
				...
				// Monto total bundles
				if(!empty($model_bundle)){
					$formula_monto_total = "=T{$current_row}*U{$current_row}";
					$calculate_sheet->setCellValue('V' . ($row_index + 1), $formula_monto_total); //Llenar columna V para los bundles
				}
		...
		if ($calculate_sheet_mix) {
			...
			foreach ($vars_mix as $model => &$model_data) {
				...
				$additional_items = array_filter($model_data, function ($item) {
					return isset($item['priority']) && $item['priority'] === '2-ADDITIONAL';
				});
				$additional_items = array_values($additional_items);

				foreach ($mix_items as $mix_item) {
					foreach ($basic_items as $basic_item) {
						if (isset($mix_item['end_date']) && isset($basic_item['end_date']) && $mix_item['start_date'] <= $basic_item['end_date']) {
						//if (isset($mix_item['end_date']) && isset($basic_item['end_date']) && $mix_item['end_date'] <= $basic_item['end_date']){
							...
							if (method_exists($this, 'qty_calculate')) {
								$start_date_mix = explode("-", $mix_values[16]);
								$start_date_mix = $start_date_mix[0].$start_date_mix[1].$start_date_mix[2];
								
								$end_date_mix = explode("-", $mix_values[17]);
								$end_date_mix = $end_date_mix[0].$end_date_mix[1].$end_date_mix[2];
								
								//echo '<pre>'; print_r([$start_date_mix, $end_date_mix]);
								//echo '<pre>'; print_r($price_promotion);
								$adj_qty = $this->qty_calculate($out_map, $mix_item['model'], $start_date_mix, $end_date_mix, 1, $price_promotion);
								//echo '<pre>'; print_r($adj_qty);
							...
							//if (!isset($count_mix)) $count_mix = 0;
							//$count_mix += 1; // Conteo de filas por modelo de mix_detail
						}
					}
					//if ($count_mix != 2){
						foreach ($additional_items as $additional_item) {
							//if (isset($mix_item['end_date']) && isset($additional_item['end_date']) && $mix_item['start_date'] <= $additional_item['end_date'] && $mix_item['seq'] > $additional_item['seq']){
							if (isset($mix_item['end_date']) && isset($additional_item['end_date']) && $mix_item['start_date'] <= $additional_item['end_date']) {
							//if (isset($mix_item['end_date']) && isset($additional_item['end_date']) && $mix_item['start_date'] >= $additional_item['start_date']){
								$mix_values = [
									$mix_item['customer'] ?? '',        // A (0)
									$mix_item['model'] ?? '',           // B (1)
									$mix_item['seq'] ?? '',             // C (2)
									...
									str_replace(',', '.', $mix_item['cost_prom'] ?? ''),    // H (7)
									$mix_item['diff'] ?? '',           // I (8)
									$additional_item['seq'] ?? '',           // J (9)
									$additional_item['start_date'] ?? '',    // K (10)
									$additional_item['end_date'] ?? '',      // L (11)
									$additional_item['qty'] ?? '',           // M (12)
									str_replace(',', '.', $additional_item['cost_sellin'] ?? ''), // N (13)
									str_replace(',', '.', $additional_item['cost_prom'] ?? ''),    // O (14)
									$additional_item['diff'] ?? '',          // P (15)
									($mix_item['start_date'] > $additional_item['start_date'] ? $mix_item['start_date'] : $additional_item['start_date']) ?? '', // Q (16)
									($mix_item['end_date'] < $additional_item['end_date'] ? $mix_item['end_date'] : $additional_item['end_date']) ?? '',    // R (17)
									0, // Placeholder para adj_qty (S - 18)
									'', // Placeholder para columna T (19)
									str_replace(',', '.', $additional_item['cost_sellin'] ?? ''), // U (20)
									str_replace(',', '.', $additional_item['cost_prom'] ?? ''),    // V (21)
									(floatval(str_replace(',', '.', $additional_item['cost_sellin'] ?? '0')) - floatval(str_replace(',', '.', $additional_item['cost_prom'] ?? '0'))), // W (22)
									0, // Placeholder para adj_total (X - 23)
								];
	...
	public function downloadSpreadsheet($spreadsheet, $file_name_param) {
        ...
        // Si $file_name_param ya incluye la extensión '.xlsx', simplemente úsalo.
        // De lo contrario, añádelo aquí.
        ...
        $upload_dir = FCPATH . 'upload_file/Sales Admin/';
        ...
        try {
            ...
            $response = [
                ...
                // Codifica el nombre del archivo para asegurar que la URL sea válida
                'downloadUrl' => base_url('upload_file/Sales Admin/' . rawurlencode($file_name)),
                // --- ¡NUEVO! Devolvemos el nombre del archivo generado para el borrado ---
                'fileNameToDelete' => $file_name 
            ];
            
            // ¡Importante! Asegúrate de que esta línea esté descomentada y al principio de la respuesta.
            header('Content-Type: application/json'); 
            echo json_encode($response);
            
            // Es buena práctica limpiar el búfer y finalizar la conexión rápidamente
            // para que la respuesta JSON llegue al cliente sin demoras.
            // if (ob_get_level()) {
                // ob_end_clean(); // Vacía y desactiva cualquier búfer de salida activo
            // }
            flush();
            if (function_exists('fastcgi_finish_request')) {
                fastcgi_finish_request(); // Ideal para PHP-FPM
            }
            
        } catch (Exception $e) {
            // Manejo de errores en caso de fallo al generar/guardar el Excel
            if (ob_get_level()) {
                ob_end_clean();
            }
            $response = [
	...
	public function delete_temp_excel_file(){
        ...
        if ($this->input->is_ajax_request() && $this->input->post('fileName')) {
            ...
            $filePath = FCPATH . 'upload_file/Sales Admin/' . $fileNameToDelete;
	...
	public function process($file_ext){	
		...
		//truncate all rows
		$this->gen_m->delete('sa_calculate_promotion', ['pr' => $emp_number]);
		$this->gen_m->delete('sa_sell_in_promotion', ['pr' => $emp_number]);
		$this->gen_m->delete('sa_sell_out_promotion', ['pr' => $emp_number]);
		...
		$this->load_calculate($spreadsheet->getSheetByName("CALCULATE"), $emp_number);
		
		// Funcion para carga hoja SELLOUT  a la DB
		$this->load_sell_out($emp_number);
		//$this->load_sell_out($spreadsheet->getSheetByName("SELLOUT"));
		
		// Funcion para cargar hoja SELLIN a la DB
		$this->load_sell_in($spreadsheet->getSheetByName("SELLIN"), $emp_number);
	...
	public function get_uploaded_files() {
		...
		if (is_dir($folder_path)) {
			foreach (scandir($folder_path) as $file) {
                ...
                // --- ¡NUEVA CONDICIÓN PARA IGNORAR ARCHIVOS! ---
                // Ignorar archivos que empiezan con "[Process]"
