...
class Sales_order extends CI_Controller {
	...
	public function process_sales_order($sheet){
		set_time_limit(0);
		...
		for ($row = 2; $row <= $max_row; $row++){
			...
			//save rowdata in text
			$aux_f = ["order_no" => $order_no, "line_no" => $line_no];
			$aux = [
				"is_so"			=> true,
				"order_no"		=> $order_no,
				"line_no"		=> $line_no,
				"order_date"	=> $order->order_date,
				"so_data"		=> implode("¦¦", $rowdata),
			];
			
			$order_txt = $this->gen_m->filter("order_txt", true, $aux_f);
			if ($order_txt) $this->gen_m->update("order_txt", $aux_f , $aux); //update
			else $this->gen_m->insert("order_txt", $aux); //insert
		}
		
		return (microtime(true) - $start_time);
	}
	
	public function process_closed_order($sheet){
		set_time_limit(0);
		$start_time = microtime(true);
		...
		for ($row = 2; $row <= $max_row; $row++){
			...
			$aux = [
				...
				"line_no"		=> $line_no,
				"order_date"	=> $order->order_date,
				"closed_date"	=> $order_item_arr["closed_date"],
				"co_data"		=> implode("¦¦", $rowdata),
			];
	...
	public function process_order_file($order_file = './upload/scm_order_file.xls'){
		$spreadsheet = IOFactory::load($order_file);
	...
	public function upload_sales_order(){
		...
		if ($this->session->userdata('logged_in')){
			...
			if ($this->upload->do_upload('file_order')){
				...
				$runtime = $this->process_order_file('./upload/'.$data['orig_name']);
