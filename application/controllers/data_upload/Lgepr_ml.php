...
class Lgepr_ml extends CI_Controller {
	...
	public function index(){
		...
		$data = [
			"ml"	=> $this->gen_m->filter("lgepr_ml", false, null, null, null, "", 100),
			"main" 		=> "data_upload/lgepr_ml/index",
		];
	...
	public function process(){
		...
		$spreadsheet = IOFactory::load("./upload/lgepr_ml.xlsx");
		...
		$data_ml = $this->gen_m->filter_select('lgepr_ml', false, $filter_select);
					...
					if (!$is_row_empty) { // Si la fila no está vacía, procesarla
						...
						foreach($data_ml as $item_ml){
							...
							if ($row['division'] === $item_ml->division && $row['yyyy'] == $item_ml->yyyy &&  $row['mm'] == $item_ml->mm &&  $row['country'] === $item_ml->country){
								...
								$this->gen_m->update("lgepr_ml", $where, $row);
								break;
							}
						...
						if ($flag_unique == 1){
							...
							if(count($batch_data)>=$batch_size){
								$this->gen_m->insert_m("lgepr_ml", $batch_data);
								$batch_data = [];
								unset($batch_data);
							}
						} else continue;
