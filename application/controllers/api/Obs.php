...
class Obs extends CI_Controller {
	...
	public function nsp(){
		...
		//aux array setting
		$s = ["model_category", "model", "bill_to_name", "close_date", "sum(sales_amount) as sales_amount", "sum(ordered_qty) as ordered_qty"];
		$w = ["close_date >= " => $from, "close_date <= " => $to, "sales_amount >" => 0];
		
		$rawdatas = $this->gen_m->only_multi("v_obs_sales_order", $s, $w, ["model_category", "model", "close_date"]);
